...
import {
  ...
  FormDataMap,
} from '../../core/model/backgroundAdultData.model';
import {
  adultCaseList,
  estimatesAdultPath,
  estimatesPath,
...
export default function BackgroundAdultDataSurveyPage() {
  ...
  const {
    data: backgroundAdultData,
    isFetched,
    refetch,
  } = useBackgroundAdultData(codeNumber);
  const {
  ...
  const [yearOfBirth, setYearOfBirth] = useState<number | null>(
    backgroundAdultData?.yearOfBirth as number
  );
  const [country, setCountry] = useState<null | string>(
    backgroundAdultData?.country || 'Sweden'
  );
  const strDate = dayjs().format('YYYY-MM-DD');

  useEffect(() => {
    refetch();
    setYearOfBirth(backgroundAdultData?.yearOfBirth as number);
  }, [backgroundAdultData]);
  ...
  useEffect(() => {
    if (
      ...
      Object.keys(backgroundAdultSurveyBasicData).forEach(
        (entityPluralName) => {
          const entityName = entityPluralName.replace('Entities', '');
          mappedData[entityName] = {};
          const basicDataUnits =
            backgroundAdultSurveyBasicData[
              entityPluralName as keyof BackgroundAdultSurveyBasicData
            ];

          basicDataUnits.forEach((basicDataUnit) => {
            const nId = basicDataUnit.id;
            const textValue =
              (backgroundAdultData?.formDataByEntityName[entityName]
                ?.filter((val) => typeof val === 'string')
                .at(0) as string) || '';

            const booleanValue =
              backgroundAdultData?.formDataByEntityName[entityName]?.includes(
                nId
              ) || false;
  ...
  const handleSave = async () => {
    if (formData) {
      try {
        ...
        newBackgroundAdultData.date = backgroundAdultData?.date ? backgroundAdultData?.date : strDate;
        ...
        console.log('unfilledEntities', unfilledEntities);
