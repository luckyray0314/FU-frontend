...
export default function SystematicFollowUpPage() {
  ...
  const { backgroundSurveyFormMetadata } = useAppSelector(
    (state) => state.backgroundSurvey
  );
  const { importantEventsFormMetadata } = useAppSelector(
    (state) => state.importantEvents
  );
  ...
  const handleSelectionImportantEventFilterChange = (
    ...
    setFiltersByEntityName({
      ...
      [activeFilter]: {
        ...
        importantEvent: {
          ...filtersByEntityName[activeFilter]?.importantEvent,
          [formMetadata.entityName]: importantEventsFormMetadata
            .filter((d) => d.entityName === formMetadata.entityName)
            ?.at(0)
            ?.entitiesData.filter((d) => newVal.includes(d.id)),
        },
  ...
  useEffect(() => {
    if (backgroundSurveyFormMetadata && importantEventsFormMetadata) {
      const initialBackgroundFilters = backgroundSurveyFormMetadata.reduce(
        (prev, next) => {
          return {
            ...prev,
            [next.entityName]: [],
      ...
      const initialImportantEventFilters = importantEventsFormMetadata.reduce(
        (prev, next) => {
          return {
            ...prev,
            [next.entityName]: [],
  ...
  }, [backgroundSurveyFormMetadata, importantEventsFormMetadata]);
  ...
  return (
    <DashboardLayout>
      ...
      <Stack direction='row' spacing={20} paddingTop={2}>
        <Stack direction='row'>
          <NavLink
            to={adultSystematicFollowUpPath()}
      ...
      <Stack direction='row' spacing={2} paddingTop={2}>
        <Stack spacing={3} flex={1} sx={{ maxWidth: '70%' }}>
        ...
        <Stack alignItems='center' spacing={2} sx={{ maxWidth: '30%' }}>
          ...
          <Stack direction='row' spacing={2}>
            <Card>
              ...
              />
              <CardContent>
                {backgroundSurveyFormMetadata?.map(
            ...
            <Card>
              ...
              />
              <CardContent>
                {importantEventsFormMetadata?.map((formMetadata, formIndex) => {
                  return (
                    <Accordion key={formIndex}>
                      <AccordionSummary>
                        <Typography color='success.main'>
